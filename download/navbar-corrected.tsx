...
export function Navbar({ onCartClick, onProfileClick }: { onCartClick?: () => void; onProfileClick?: () => void }) {
  ...
  const openMega = () => {
    ...
    if (!hoveredCat) setHoveredCat(catSlugs[0]);
    setMegaOpen(true);
  };
  const closeMega = () => {
  ...
  /* ─── Nav item style ─── */
  const navItemClass = 'text-[11px] font-semibold text-white/70 hover:text-[#D4AF37] transition-colors tracking-wide uppercase px-3 py-1.5 rounded-md hover:bg-white/5';
      ...
      <nav className={'sticky top-0 z-50 transition-all duration-300 ' + (scrolled ? 'bg-[#0B0B0B]/95 backdrop-blur-xl shadow-lg shadow-black/30 border-b border-[#1a1a1a]' : 'bg-[#0B0B0B] border-b border-[#1a1a1a]')}>
        <div className="max-w-[1400px] mx-auto px-4 lg:px-6">
          ...
          <div className="flex items-center justify-between h-12 lg:h-14">
            ...
            {/* Search */}
            <div className="hidden md:flex flex-1 max-w-xl mx-6 items-center bg-[#111] border border-[#222] rounded-full px-5 py-2 gap-2 focus-within:border-[#D4AF37]/50 transition-colors">
          ...
          {/* ───── LIGNE 2 : Navigation ───── */}
          <div className="hidden lg:flex items-center gap-1 border-t border-[#1a1a1a]/50 py-1.5">
            ...
            >
              <a
                ...
                className="text-[11px] font-semibold text-[#D4AF37] hover:text-[#F5E6A3] transition-colors tracking-wide uppercase flex items-center gap-1 px-3 py-1.5 rounded-md hover:bg-[#D4AF37]/5"
              >
                Boutique
                <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={'transition-transform duration-200 ' + (megaOpen ? 'rotate-180' : '')}><path d="m6 9 6 6 6-6"/></svg>
              </a>
            </div>
            <a href="#categories" className={navItemClass}>
              {'Cat\u00e9gories'}
            </a>
            <button
              onClick={() => goToCategory('auto-moto')}
              className={navItemClass + ' flex items-center gap-1'}
            >
              {'\u{1F697}'} Auto
            </button>
            <a href="#promotions" className={navItemClass}>
              Promos
            </a>
            <button
              onClick={() => goToCategory('bricolage')}
              className={navItemClass + ' flex items-center gap-1'}
            >
              {'\u{1F9F0}'} Outils
            </button>
        ...
        {/* ───────────────────────────────────────
            ...
            Panneau droite: sous-catégories
        ─────────────────────────────────────── */}
        {megaOpen && (
          <div
          ...
          >
            <div className="max-w-[1400px] mx-auto flex">
              ...
              <div className="w-[220px] shrink-0 border-r border-[#1a1a1a] py-4 px-2 max-h-[520px] overflow-y-auto">
                ...
                {catSlugs.map((slug) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-xs font-medium leading-tight">{catNames[slug] || slug}</span>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={'ml-auto shrink-0 transition-colors ' + (isActive ? 'text-[#D4AF37]' : 'text-white/15')}><path d="m9 18 6-6-6-6"/></svg>
                    </button>
                  );
                })}
              </div>
              ...
              {/* ─── PANNEAU DROITE ─── */}
              <div className="flex-1 py-6 px-6 max-h-[520px] overflow-y-auto">
                {hoveredCat && shopCategoriesData[hoveredCat] ? (
                  hasDetail(hoveredCat) ? (
                    <div className="flex h-full">
                      ...
                      {/* Zone principale: items du groupe */}
                      <div className="flex-1 min-w-0">
                        {hoveredGroup && getDetail(hoveredCat)?.groups.find((g) => g.name === hoveredGroup) ? (
                          <div>
                            <div className="flex items-center justify-between mb-3">
                              <h4 className="text-sm font-bold text-white">{hoveredGroup}</h4>
                              ...
                              {getDetail(hoveredCat)!
                                ...
                                .items.map((item) => (
                                  <button
                                    key={item}
                                    onClick={() => goToCategory(hoveredCat)}
                                    className="text-left py-1 text-xs text-white/55 hover:text-[#D4AF37] transition-colors duration-150 truncate"
                                  >
                                    {item}
                                  </button>
                                ))}
                          ...
                          <div className="flex flex-col items-center justify-center h-full text-white/20">
                            ...
                            {'Survolez un groupe pour voir les d\u00e9tails'}
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      {shopCategoriesData[hoveredCat].subCategories && shopCategoriesData[hoveredCat].subCategories!.length > 0 ? (
                        <div className={'grid gap-x-8 gap-y-0.5 ' + (
        ...
        {mobileOpen && (
          <div className="lg:hidden bg-[#0B0B0B]/98 backdrop-blur-xl border-t border-[#1a1a1a]">
            <div className="px-4 py-4 space-y-3">
              ...
              <button onClick={() => goToCategory('auto-moto')} className="block py-2.5 text-white/80 hover:text-[#D4AF37] transition-colors font-medium tracking-wide uppercase text-sm w-full text-left">{'\u{1F697}'} Auto</button>
              ...
              <button onClick={() => goToCategory('bricolage')} className="block py-2.5 text-white/80 hover:text-[#D4AF37] transition-colors font-medium tracking-wide uppercase text-sm w-full text-left">{'\u{1F9F0}'} Outils</button>
