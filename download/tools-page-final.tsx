...
export default function ToolsPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      {toast && (
        <div className="fixed top-6 right-6 z-[100] bg-[#D4AF37] text-black font-bold px-6 py-3 rounded-xl shadow-2xl shadow-[#D4AF37]/20" style={{animation:'slideIn 0.3s ease'}}>
      ...
      <div className="flex min-h-screen">
        ...
        <main className="flex-1 min-w-0">
          <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {/* GRILLE 18 CATEGORIES */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
              {filteredCategories.map(cat => (
                <div
                  key={cat.id}
                  id={'cat-' + cat.id}
                  className="bg-[#111] rounded-xl border border-[#1a1a1a] overflow-hidden hover:border-[#222] transition-colors duration-200"
                ...
                >
                    ...
                    <div className="space-y-0.5">
                      {cat.tools.slice(0, 8).map((tool, i) => (
                        <button
                          ...
                          className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-left text-xs text-white/50 hover:text-white hover:bg-white/[0.04] transition-all duration-100"
                        >
                  ...
                  <div className="px-4 pb-4">
                    <button onClick={() => showToast(cat.name)} className="text-[11px] font-semibold hover:underline transition-colors" style={{ color: cat.color }}>
            ...
            {/* SECURITE + AVANTAGES + COMMENT CA MARCHE */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
              ...
              <div className="bg-[#111] border border-[#1a1a1a] rounded-xl p-5">
                ...
                <div className="space-y-2.5">
                  {['Fichiers supprimés automatiquement','Aucun fichier stocké définitivement','Connexion sécurisée SSL / HTTPS','Vos fichiers ne sont jamais partagés','Respect total de la vie privée'].map((item, i) => (
              ...
              <div className="bg-[#111] border border-[#1a1a1a] rounded-xl p-5">
                ...
                <div className="space-y-2.5">
                  {['Rapide et stable',"Peut supporter des millions d'utilisateurs",'Pas de bug même en forte charge','Solide et fiable','Entièrement sécurisé'].map((item, i) => (
                    <div key={i} className="flex items-start gap-2.5">
        ...
        <aside className="hidden xl:block w-[250px] shrink-0 bg-[#0a0a0a] border-l border-[#1a1a1a] sticky top-0 h-screen overflow-y-auto">
          <div className="p-4">
            ...
            <div className="space-y-1">
              {[
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left hover:bg-white/[0.03] transition-colors"
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-medium text-white/70 hover:text-white truncate">{item.name}</p>
            ...
            <button onClick={() => showToast('Historique')} className="w-full py-2.5 rounded-lg border border-[#222] text-xs text-white/40 hover:text-white hover:border-[#333] transition-colors text-center">
